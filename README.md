...
- **Vitest 单元测试**（26 个文件共 202 条用例）：
  ...
  - 考试倒计时（自然日天数与跨月跨年、不存在的日期、首页排序、「日期 + 开考时间」拼接与保存前校验）
...
| 首页             | 按时段问候（早上好 / 下午好…）、按角色的功能宫格、**今日课程**（学生/教师：时间轴彩色块，实时「进行中/未开始/已结束」状态）、教务通知、考试安排（即将开考的排最前，带倒计时标签）、考勤统计（环形图）、成绩统计（柱状图）；数据本地缓存秒开（按账号隔离）+ 无缓存时骨架屏                                                                                                                                                                      |
| 智能推荐         | 基于物品的协同过滤课程推荐（学生角色）：按选课相似度生成「为你推荐」并附推荐理由，冷启动降级热门课程，点击直达选课页                                                                                                                                                                                                                                                                                                                           |
| 信息公告         | 教务通知 / 考试安排（**考试倒计时**：今天 / 明天 / 还有 N 天 / 已结束，3 天内红色、一周内橙色；管理员新增时选考试日期 + 开考时间）/ **教室信息管理**（编号/类型/固定占用/容纳人数，管理员维护）                                                                                                                                                                                                                                                |
...
转发、新版本提示只在微信里有，H5 预览测不到；原生选择器在 H5 里是 uni-app 模拟的，也以开发者工具 / 真机为准：

- 转发：页面右上角「…」→「转发」应可点；通知 / 考试安排 / 教室 / 课程页转发的是当前页，其余页面转发首页
- 考试时间：管理员新增考试安排，「考试日期」「开考时间」两个原生选择器都能选中并回显，只选一个时保存会提示「请选择考试时间」；保存后列表与首页出现倒计时标签
...
3. 接口地址在 `.env.development` / `.env.production` 的 `VITE_API_BASE_URL`（由 `src/utils/config.ts` 读取，改环境只改 env 文件）：
   ...
   - 正式发布：改为已备案的 https 域名（构建时会校验，见「工程化」的发布前地址校验，用 `RELEASE=1 npm run build:mp-weixin` 构建），并在公众平台「开发管理 → 服务器域名」登记：request / uploadFile / downloadFile 合法域名填 `https://` 地址，socket 合法域名填 `wss://` 地址（实时通知走 WebSocket，作业附件查看走下载）
...
4. 数据库使用 Web 端仓库的最新种子 `manager-vue3/sql/xm_educational_manager-full.sql`（已含 `theme` / `locale` / `theme_color` / `exam_time` 列与 RBAC 授权）；已经在用的旧库不必重导，新增的列由后端启动时自动补齐（`SchemaMigration`），只有教师课程编辑授权需手动补一条（见「已知取舍」）。表结构与字段含义见 `manager-vue3/docs/数据库ER图.md`。
...
|   +-- xm-picker/              # 表单下拉（v-model：枚举 / 接口列表 / 日期 / 时间四种用法，空值与失效值显示占位）
...
|   +-- examCountdown.js        # 考试倒计时纯函数（自然日天数、文案与标签色、首页排序、日期 + 开考时间拼接；与 Web 端同规则）
...
tests/                          # Vitest 单元测试（26 个文件 202 条，setup.js 提供全局 uni mock）
```
...
- 下拉 / 日期控件 → 原生选择器（统一经 `xm-picker` 封装，日期为 `mode="date"`、时间为 `mode="time"`，后端仍按字符串存储，两端行为一致）；Web 端的日期时间选择器 → 小程序没有日期时间合一的原生选择器，拆成「考试日期 + 开考时间」两个，拼成同样的 `yyyy-MM-dd HH:mm`；附件选择 → `uni.chooseImage` + `uni.uploadFile`。
...
- **考试倒计时按自然日计算**：只比日期（今天 / 明天 / 还有 N 天），不精确到小时，当天的考试开考后仍显示「今天」，次日才变「已结束」。考试时间是新增字段（`exam_time`，后端启动时自动给旧库补列），升级前录入的考试安排没有考试时间：列表显示「未设置考试时间」、不显示倒计时，首页排在最后并显示发布时间；编辑保存时需补选考试时间。
