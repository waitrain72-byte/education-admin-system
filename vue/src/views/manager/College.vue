<template>
  <div>
    <div class="search">
      <el-input v-model="name" :placeholder="$t('pages.college.searchPlaceholder')" style="width: 200px" />
    ...
    <el-dialog v-model="formVisible" :title="$t('pages.college.dialogTitle')" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form ref="formRef" label-width="100px" style="padding-right: 50px" :model="form" :rules="rules">
        <el-form-item prop="name" :label="$t('pages.college.name')">
          <el-input v-model="form.name" autocomplete="off" />
        </el-form-item>
        <el-form-item prop="content" :label="$t('pages.college.content')">
...
const {
  ...
  url: '/college',
  rules: computed(() => ({
    name: [{ required: true, message: t('pages.college.ruleNameRequired'), trigger: 'blur' }],
  })),
  getParams: () => ({ name: name.value }),
})

const columns = computed<CrudColumn[]>(() => [
  { prop: 'id', label: t('pages.college.id'), width: 80, align: 'center', sortable: true },
  { prop: 'name', label: t('pages.college.name'), showOverflowTooltip: true },
  { prop: 'content', label: t('pages.college.content'), showOverflowTooltip: true },
])
