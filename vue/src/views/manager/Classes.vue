<template>
  <div>
    <div class="search">
      <el-input placeholder="请输入班级名称" style="width: 200px" v-model="name" />
    ...
    <div class="table">
      <el-table :data="tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="content" label="班级描述" show-overflow-tooltip />
        <el-table-column prop="specialityName" label="所属专业" show-overflow-tooltip />
...
const loadSpeciality = () => {
  request.get('/speciality/selectAll').then((res: any) => {
    if (res.data.code === '200') { specialityData.value = res.data.data }
    else { ElMessage.error(res.data.msg) }
  })
}
const loadTeacher = () => {
  request.get('/teacher/selectAll').then((res: any) => {
    if (res.data.code === '200') { teacherData.value = res.data.data }
    else { ElMessage.error(res.data.msg) }
  })
}
...
const save = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      request({ url: form.value.id ? '/classes/update' : '/classes/add', method: form.value.id ? 'PUT' : 'POST', data: form.value })
        .then((res: any) => {
...
const del = (id: number) => {
  ElMessageBox.confirm('您确定删除吗？', '确认删除', { type: 'warning' }).then(() => {
    request.delete('/classes/delete/' + id).then((res: any) => {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('您确定批量删除这些数据吗？', '确认删除', { type: 'warning' }).then(() => {
    request.delete('/classes/delete/batch', { data: ids.value }).then((res: any) => {
      if (res.data.code === '200') { ElMessage.success('操作成功'); load(1) }
      else { ElMessage.error(res.data.msg) }
    })
  }).catch(() => {})
...
const load = (pNum?: number) => {
  ...
  request.get('/classes/selectPage', { params: { pageNum: pageNum.value, pageSize: pageSize.value, name: name.value } })
...
onMounted(() => { load(1); loadSpeciality(); loadTeacher() })
</script>
