<template>
    <div>
        ...
        >
            <template #file="{ row }">
                <el-button type="warning" size="small" @click="down(row.file)">{{
                    $t('pages.homework.download')
                }}</el-button>
            </template>
            <template #actions="{ row }">
                <el-button
                    v-if="user.role === 'STUDENT' && !row.score"
                    link
                    type="primary"
                    size="small"
                ...
                >
                <el-button v-if="user.role === 'STUDENT'" link type="danger" size="small" @click="del(row.id)">{{
                    $t('common.delete')
                }}</el-button>
                <el-button v-if="user.role === 'TEACHER'" link type="primary" size="small" @click="handleCheck(row)">{{
        ...
        <el-dialog
            v-model="formVisible"
            :title="$t('pages.homework.dialogTitle')"
            width="40%"
...
const { tableData, pageNum, pageSize, total, loading, formVisible, form, formRef, rules, load, save, del } = useCrud({
    url: '/homework',
...
const down = (url: string) => {
    // 历史数据存的是老绝对地址：归一成 /api/ 代理路径再跳转；空附件不触发跳转（避免 location.href=undefined 变成 404）
