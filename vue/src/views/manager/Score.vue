  ...
  <div>
    ...
    <div class="table">
      <el-table :data="tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="ordinaryScore" label="平时分" show-overflow-tooltip />
        <el-table-column prop="examScore" label="期末分" show-overflow-tooltip />
...
const handleEdit = (row: any) => { form.value = JSON.parse(JSON.stringify(row)); studentId.value = form.value.studentId; fromVisible.value = true }
const save = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      form.value.studentId = studentId.value
      request({ url: form.value.id ? '/score/update' : '/score/add', method: form.value.id ? 'PUT' : 'POST', data: form.value })
        .then((res: any) => {
...
const del = (id: number) => {
  ElMessageBox.confirm('您确定删除吗？删除后该学生的学分也会相应的减少，您需要重新再次录入？', '灵魂拷问', { type: 'warning' }).then(() => {
    request.delete('/score/delete/' + id).then((res: any) => {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('您确定批量删除这些数据吗？', '确认删除', { type: 'warning' }).then(() => {
    request.delete('/score/delete/batch', { data: ids.value }).then((res: any) => {
      if (res.data.code === '200') { ElMessage.success('操作成功'); load(1) } else { ElMessage.error(res.data.msg) }
    })
  }).catch(() => {})
}
const load = (pNum?: number) => {
  ...
  request.get('/score/selectPage', { params: { pageNum: pageNum.value, pageSize: pageSize.value, courseId: courseId.value } })
