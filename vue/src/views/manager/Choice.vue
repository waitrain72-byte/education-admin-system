<template>
    <div>
        <!-- 为你推荐（基于物品的协同过滤，学生角色可见；冷启动降级为热门推荐） -->
        <el-card v-if="user.role === 'STUDENT' && recommends.length" shadow="never" style="margin-bottom: 16px">
            <template #header>
                <span>🎯 {{ t('pages.choice.recommend') }}</span>
            </template>
            <div v-for="c in recommends" :key="c.id" class="recommend-item">
                <span class="recommend-name">{{ c.name }}</span>
                <el-tag size="small" type="info" effect="plain">{{ c.type }}</el-tag>
                <span class="recommend-reason">{{ c.reason }}</span>
                <span class="recommend-meta">{{ c.teacherName }} · {{ c.credit }} {{ t('pages.choice.credit') }}</span>
            </div>
        </el-card>
        ...
        >
            <template #actions="{ row }">
                <el-button link type="danger" size="small" :disabled="row.status !== '未开课'" @click="del(row.id)">{{
                ...
                <el-button
                    ...
                    >{{ $t('pages.choice.comment') }}</el-button
                >
            </template>
        </CrudTable>
        ...
        <el-dialog
            v-model="formVisible"
            :title="$t('pages.choice.dialogTitle')"
            width="40%"
...
<style scoped>
/* 为你推荐：每行「课程名 + 类型 + 推荐理由 + 师资学分」的横向信息条 */
.recommend-item {
    display: flex;
    align-items: center;
    gap: 12px;
