<template>
  <div>
    <div class="search">
      <el-input placeholder="请输入专业名称" style="width: 200px" v-model="name" />
    ...
    <div class="table">
      <el-table :data="tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="name" label="专业名称" show-overflow-tooltip />
        <el-table-column prop="content" label="专业描述" show-overflow-tooltip />
        <el-table-column prop="collegeName" label="所属学院" show-overflow-tooltip />
        <el-table-column prop="score" label="学分限定" show-overflow-tooltip />
...
const rules: FormRules = { name: [{ required: true, message: '请输入专业名称', trigger: 'blur' }] }

const loadCollege = () => {
  request.get('/college/selectAll').then((res: any) => {
    if (res.data.code === '200') { collegeData.value = res.data.data } else { ElMessage.error(res.data.msg) }
  })
}
...
const save = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      request({ url: form.value.id ? '/speciality/update' : '/speciality/add', method: form.value.id ? 'PUT' : 'POST', data: form.value })
        .then((res: any) => {
...
const del = (id: number) => {
  ElMessageBox.confirm('您确定删除吗？', '确认删除', { type: 'warning' }).then(() => {
    request.delete('/speciality/delete/' + id).then((res: any) => {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('您确定批量删除这些数据吗？', '确认删除', { type: 'warning' }).then(() => {
    request.delete('/speciality/delete/batch', { data: ids.value }).then((res: any) => {
      if (res.data.code === '200') { ElMessage.success('操作成功'); load(1) } else { ElMessage.error(res.data.msg) }
    })
  }).catch(() => {})
}
const load = (pNum?: number) => {
  ...
  request.get('/speciality/selectPage', { params: { pageNum: pageNum.value, pageSize: pageSize.value, name: name.value } })
