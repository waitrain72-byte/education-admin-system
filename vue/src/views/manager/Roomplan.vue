<template>
  <div>
    <div class="search">
      <el-input placeholder="请输入教室名称" style="width: 200px" v-model="name" />
    ...
    <div class="table">
      <el-table :data="tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="name" label="教室名称" show-overflow-tooltip />
        <el-table-column prop="status" label="使用状态" show-overflow-tooltip />
        <el-table-column prop="content" label="使用说明" show-overflow-tooltip />
        <el-table-column prop="num" label="容纳人数" />
...
const save = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      request({ url: form.value.id ? '/roomplan/update' : '/roomplan/add', method: form.value.id ? 'PUT' : 'POST', data: form.value })
          .then((res: any) => {
...
const del = (id: number) => {
  ElMessageBox.confirm('您确定删除吗？', '确认删除', { type: 'warning' }).then(() => {
    request.delete('/roomplan/delete/' + id).then((res: any) => {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('您确定批量删除这些数据吗？', '确认删除', { type: 'warning' }).then(() => {
    request.delete('/roomplan/delete/batch', { data: ids.value }).then((res: any) => {
      if (res.data.code === '200') { ElMessage.success('操作成功'); load(1) } else { ElMessage.error(res.data.msg) }
    })
  }).catch(() => {})
}
const load = (pNum?: number) => {
  ...
  request.get('/roomplan/selectPage', { params: { pageNum: pageNum.value, pageSize: pageSize.value, name: name.value, status: status.value } })
      .then((res: any) => { tableData.value = res.data?.data?.list; total.value = res.data?.data?.total })
}
const reset = () => { name.value = null; status.value = null; load(1) }
