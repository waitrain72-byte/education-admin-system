<template>
  <div>
    <div class="search">
      <el-input placeholder="请输入标题查询" style="width: 200px" v-model="name" />
...
const save = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      request({ url: form.value.id ? '/examplan/update' : '/examplan/add', method: form.value.id ? 'PUT' : 'POST', data: form.value })
        .then((res: any) => {
...
const del = (id: number) => {
  ElMessageBox.confirm('您确定删除吗？', '确认删除', { type: 'warning' }).then(() => {
    request.delete('/examplan/delete/' + id).then((res: any) => {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('您确定批量删除这些数据吗？', '确认删除', { type: 'warning' }).then(() => {
    request.delete('/examplan/delete/batch', { data: ids.value }).then((res: any) => {
      if (res.data.code === '200') { ElMessage.success('操作成功'); load(1) } else { ElMessage.error(res.data.msg) }
    })
  }).catch(() => {})
}
const load = (pNum?: number) => {
  ...
  request.get('/examplan/selectPage', { params: { pageNum: pageNum.value, pageSize: pageSize.value, name: name.value } })
