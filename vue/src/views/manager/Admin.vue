      ...
      <el-table :data="tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" align="center" width="260">
          <template #default="scope">
            ...
            <el-button size="small" type="warning" plain @click="resetPassword(scope.row)" :disabled="user.role !== 'ADMIN' || scope.row.id === user.id">重置密码</el-button>
...
const save = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      request({
        url: form.value.id ? '/admin/update' : '/admin/add',
...
const del = (id: number) => {
  ElMessageBox.confirm('您确定删除吗？', '确认删除', { type: 'warning' }).then(() => {
    request.delete('/admin/delete/' + id).then((res: any) => {
      if (res.data.code === '200') {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('您确定批量删除这些数据吗？', '确认删除', { type: 'warning' }).then(() => {
    request.delete('/admin/delete/batch', { data: ids.value }).then((res: any) => {
      if (res.data.code === '200') {
        ElMessage.success('操作成功')
        load(1)
      } else {
